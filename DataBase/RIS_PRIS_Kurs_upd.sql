...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('Material') and o.name = 'FK_MATERIAL_PRINT_RELEASE')
alter table Material
   drop constraint FK_MATERIAL_PRINT_RELEASE
go
...
create table Comment (
   ID_comment           int                  identity,
   ID_material          int                  not null,
   ID_author            int                  not null,
   Short_descr          nvarchar(1024)       not null,
   Long_descr           nvarchar(1024)       not null,
...
create table Material (
   ...
   N_page               tinyint              null,
   Deadline_write       date             not null,
   Deadline_design      date             not null,
...
create nonclustered index Print_FK on Material (ID_release ASC)
go
...
create table Release (
   ID_release           int                  identity,
   N_release            int                  not null,
   Month                nvarchar(1024)       not null,
   Year                 int                  not null,
...
alter table Material
   add constraint FK_MATERIAL_PRINT_RELEASE foreign key (ID_release)
