...
insert into Position values (1, 'Шеф-редактор')
insert into Position values (2, 'Выпускающий редактор')
insert into Position values (3, 'Редактор номера')
insert into Position values (4, 'Бильд-редактор')
insert into Position values (5, 'Участник')

insert into Status values (1, 'Пишется')
insert into Status values (2, 'На проверку')
insert into Status values (3, 'На верстку')
insert into Status values (4, 'На заключительную проверку')
insert into Status values (5, 'Готово к печати')
...
insert into Creates (ID_member, ID_material) values (3,3) --Я верстаю сейчас этот текст
...
insert into Specialization values (1, 'Журналист')
insert into Specialization values (2, 'Верстальщик')
...
create index MemberSpec
on Member(ID_member,ID_spec)

exec MembersDoMaterial @ID_material=1, @ID_spec=5

SELECT * FROM Member WHERE Login = 'Viktoriya'
select ID_spec, Spec_name from Specialization

exec SelectHeaders @ID_member = 3

begin tran
select count(*) from Member
insert into Member (ID_spec, ID_position, Surname, Name, Patronymic, Birthday, Link_VK, Date_of_taking, Scholarship, Login, Password) 
values (1, 5, 'skjvb', 'Пkfр', 'ojhv', '11/06/2000', 'https://vk.com/pirotechvika', '01/10/2015', '10000', 'lshvl', '123')
rollback tran
select count(*) from Member
commit tran

select ID_material, Header from Material
